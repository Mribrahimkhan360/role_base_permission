    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        ...
        <div class="px-7 py-5 border-b border-slate-100 bg-gradient-to-r from-brand-50 to-white">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="font-bold text-slate-800 text-sm">New Product</p>
                    <p class="text-xs text-slate-500">Select a brand then enter the product name</p>
                </div>
            </div>
        </div>

        {{-- Form --}}
        <form action="{{ route('products.store') }}" method="POST" class="px-7 py-6 space-y-5">
            @csrf
            ...
            <div class="flex items-center gap-3 pt-2">
                <button type="submit"
                    class="flex-1 inline-flex items-center justify-center gap-2 bg-brand-600 hover:bg-brand-700 text-gray-400 font-semibold text-sm py-3 rounded-xl shadow-md shadow-brand-600/30 transition-all duration-200 hover:scale-[1.02]">
